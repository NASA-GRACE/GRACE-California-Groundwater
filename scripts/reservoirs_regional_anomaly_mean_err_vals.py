...
3. Saves the result as: anomaly_timeseries_{reservoirs_model}_{region}.csv
'''
...
def parse_arguments(options: Options) -> None:
    ...
    parser = argparse.ArgumentParser(description=f"Compute anomalies and error values for {options.reservoirs_model} regional CSVs.")
...
def process_csv(options: Options, file_path: str, output_dir: str, start_date: str, end_date: str, err_val: float) -> None:
    ...
    df[['date','anomaly', 'error']].to_csv(output_file, mode="a", index=False)
