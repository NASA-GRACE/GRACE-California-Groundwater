...
def mask_timeseries_for_NLDAS(options: Options) -> None:
    ...
    output_csv(options, options.args.output_csv, fieldnames, ds.time.data, anomalies_dict,
               global_attrs=global_attrs, total_area=sum(areas) * options.area_m2_scale)
...
def output_csv(options: Options, output_file: str | os.PathLike[str], fieldnames: list[str],
               ...
               total_area: float | None = None) -> None:
    ...
    Args:
        ...
        total_area:     Optional total area in m² to include in the global attributes.
